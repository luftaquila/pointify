...
async fn main(_spawner: Spawner) -> ! {
    ...
    // === Detect supply voltage via internal Vrefint ===
    ...
    // VDD_mV = VREF_INT(1200) * ADC_MAX(4095) / vref_raw
    // 3.3V → ~1489, 5V → ~983, threshold ~1200
    let is_5v_supply = vref_raw < 1200;
    println!(
        "Supply: {}V (Vrefint raw={})",
        if is_5v_supply { "5" } else { "3.3" },
        vref_raw
    );

    // === PWM Setup ===

    // TIM2 remap=0: PWM1(PA0), PWM2(PA1), PWM3(PA2), PWM4(PA3)
    ...
    // TIM1 remap=1: PWM10(CH1N=PA7), PWM9(CH3N=PB1)
    ...
    // === USB CDC Setup ===
    usb_cdc::usb_init();
    println!("Pointify firmware started");

    // Cache max duty values
    ...
    loop {
        if let Some(len) = usb_cdc::usb_poll(&mut rx_buf) {
            for chunk in rx_buf[..len].chunks_exact(2) {
                ...
                // 5V supply + 3V gauge: scale so 1023 → 3V (60% of 5V)
                let scale_3v = is_5v_supply && !is_5v_gauge;

                match index {
                    0 => pwm_tim2.set_duty(
                        Channel::Ch1,
                        duty_u32(value, max_duty_tim2, scale_3v),
                    ),
                    1 => pwm_tim2.set_duty(
                        Channel::Ch2,
                        duty_u32(value, max_duty_tim2, scale_3v),
                    ...
                    4 => pwm_tim3.set_duty(
                        Channel::Ch1,
                        duty_u32(value, max_duty_tim3, scale_3v),
                    ),
                    8 => pwm_tim1.set_duty(
                        Channel::Ch3,
                        duty_u16(value, max_duty_tim1, scale_3v),
                    ),
                    9 => pwm_tim1.set_duty(
                        Channel::Ch1,
                        duty_u16(value, max_duty_tim1, scale_3v),
